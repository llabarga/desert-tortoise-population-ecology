...
# Creem una llista buida per a guardar els resultats
...
for(sim in 1:n_sim) {
  ...
  for (time_step in 1:n_temps) {
    ...
    # Aquí introduïm estocasticitat en 8 paràmetres de la matriu
    # Augmentem la supervivencia de totes les classes i la reproducció
    # perquè és la integració de totes les mesures
    # Cada any que passa, aquests valors es mostrejen d'una distribució
    # normal, truncada (per no triar valors erronis)
    
    r1<-rtruncnorm(1, mean = 0.2, sd = 0.1,a=0, b=1)
    r2<-rtruncnorm(1, mean = 0.15, sd = 0.1,a=0, b=1)
    a1<-rtruncnorm(1, mean = 0.2, sd = 0.1,a=0, b=1)
    a2<-rtruncnorm(1, mean = 0.99, sd = 0.1,a=0, b=1)
    a3<-rtruncnorm(1, mean = 0.99, sd = 0.1,a=0, b=1)
...
# Gràfics de totes les simulacions
...
ggplot(pobdf_resum,aes(x=Any,y= Ntotal))+
  geom_ribbon(aes(ymin=Ntotal-SDtotal,ymax=Ntotal+SDtotal),fill='light yellow')+
