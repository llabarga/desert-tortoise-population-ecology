...
for(sim in 1:n_sim) {
  ...
  for (time_step in 1:n_temps) {
    ...
    # Augmentem la reproducció de les classes madures
    # Cada any que passa, aquests valors es mostrejen d'una distribució
    # normal, truncada (per no triar valors erronis)
    
    r1<-rtruncnorm(1, mean = 0.2, sd = 0.1,a=0)
    r2<-rtruncnorm(1, mean = 0.15, sd = 0.1,a=0)
...
ggplot(pobdf_resum,aes(x=Any,y= Ntotal))+
  geom_ribbon(aes(ymin=Ntotal-SDtotal,ymax=Ntotal+SDtotal),fill='light pink')+
  geom_line(aes(y=Ntotal))+ xlab("Any") + ylab("Nº total d'individus")+ ggtitle("Escenari 3")
